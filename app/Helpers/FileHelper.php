...
class FileHelper
{
    ...
    public function getImageDescriptors(Request $request)
    {
        $imagePath = public_path('path/to/your/image.jpg'); // Путь к изображению

        $image = OpenCV::load($imagePath);
        $grayImage = $image->cvtColor(OpenCV::COLOR_BGR2GRAY);

        $sift = OpenCV::SIFT();
        $keypointsSIFT = $sift->detect($grayImage);
        $descriptorsSIFT = $sift->compute($grayImage, $keypointsSIFT);

        $surf = OpenCV::SURF();
        $keypointsSURF = $surf->detect($grayImage);
        $descriptorsSURF = $surf->compute($grayImage, $keypointsSURF);

        return response()->json([
            'sift_keypoints' => $keypointsSIFT,
            'sift_descriptors' => $descriptorsSIFT,
            'surf_keypoints' => $keypointsSURF,
            'surf_descriptors' => $descriptorsSURF,
        ]);
    }
}
