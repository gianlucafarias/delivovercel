    ...
    public function update(Receipt $model, array $data): array
    {
        try {
            ...
            DB::transaction(function () use ($model, $data) {
                ...
                if (data_get($data, 'stocks.*.stock_id')) {
                    DB::table('receipt_stocks')
                        ->whereIn('stock_id', data_get($data, 'stocks.*.stock_id'))
                        ->delete();
                }
        ...
        foreach ($models as $model) {
            ...
            try {
                DB::transaction(function () use ($model) {
                    ReceiptStock::whereIn('stock_id', $model->stocks->pluck('id')->toArray())->delete();
