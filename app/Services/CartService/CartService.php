...
//        if ($stock?->countable?->interval > 1) {
//            $intervalQuantity = $quantity / 1000;
//        }

        data_set($data, 'price', ($stock->price + $stock->tax_price) * $intervalQuantity);
        data_set($data, 'discount', $stock->actual_discount * $intervalQuantity);
...
//        }
    ...
    public function calculateTotalPrice(Cart $cart): bool
    {
        ...
        if (!empty($bonus) && (!$bonus->status || $bonus->value > $price && $bonus->type === Bonus::TYPE_SUM)) {
